...
def evaluate( model , dataset , config):
    ...
    #with open('original_keypoints.json','w') as f:
        #json.dump(predict_results, f)
    logger.info('\n=+=+= the number of predict_results samples is {} before OKS NMS \n'.format(len(predict_results)))
    ...
    for img in image_kpts.keys():
        ...
        ## oks nms
        kpts = oks_nms(kpts, threshold=config.test.oks_nms_threshold)
    ...
    logger.info('\n=+=+=  the number of predict_results samples is {} after OKS NMS \n'.format(len(dt_list)))
